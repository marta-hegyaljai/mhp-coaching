...
test("case library and insights preview the forthcoming work", async ({page}) => {
  ...
  await expect(page.locator("header").first().getByRole("link", {name: "Case Library"})).toHaveAttribute(
    "aria-current",
    "page",
  );
...
test("the about page publishes the founder portrait, quote and recognitions", async ({page}) => {
  ...
  await expect(page.locator("header").first().getByRole("link", {name: "À propos"})).toHaveAttribute(
    "aria-current",
    "page",
  );
