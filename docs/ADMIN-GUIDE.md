...
1. **Calls → Messages**, or a message row on the control panel, opens the
   inbound course question, contact-form or other-payment-method record.
...
3. Someone who asked to pay another way is a Payment help row: the written
   payment form if they sent one, otherwise the `LEAD` booking. Enrolment
   lists and the control panel also offer Reply by email on those leads.
4. Unanswered rows stay gold. After a send they read as Replied.
